...
class ukyDataSet:
    ...
    def computeScore(self, split, check_valid=True):
        if check_valid and not self.validSplit(split):
            return 2.0,
        else:
    ...
    def get_validation_weights(self, training_indices, validation_indices):
        weights = self.ratios(np.isin(np.arange(self.size), training_indices))
